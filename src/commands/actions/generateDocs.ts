...
export async function generateDocs(
  ...
  const strategyKey = process.env.USE_FULL_CONTEXT
    ? "Full Context Doc Generation"
    : "Smart Doc Generation";

  loading.stop(
    `Picked strategy: ${strategyKey}\nParallel Info: Using chunk size of ${chunkSize}`
  );
  ...
  const recursivePromise = (chunk: TreeItemFlatted[], i: number) => {
    const promise = Promise.all(
      chunk.map(async item => {
        ...
        } else handleLoading(i, `Skipped ${item.fullPath}`);
      })
    );
